  ...
  return (
    <div className="rounded-2xl p-4 bg-[var(--color-gold)]/10 border border-[var(--color-gold)]/30">
      <div className="flex items-center gap-2 mb-2.5">
        ...
        <h3 className="text-sm font-bold text-white">Пора списать в этом месяце</h3>
      </div>
      <div className="space-y-1.5">
        {due.map((r) => {
          const notEnough = r.amount > state.balance
          return (
            <div key={r.id} className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-[var(--color-gold)] text-[var(--color-bg-deep)] font-bold text-xs rounded-lg hover:brightness-110 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed transition"
              >
                Записать
              </button>
            </div>
