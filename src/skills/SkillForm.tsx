...
export function SkillForm({ initial, onSubmit, onDelete }: SkillFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ title: trimmedTitle, emoji: trimmedEmoji })
  }

  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <label className="block">
        <span className="text-sm text-white/70 font-semibold mb-1.5 block">Название навыка</span>
        <input
          type="text"
          value={title}
      ...
      >
        {initial ? 'Сохранить' : 'Создать навык'}
      </button>
      ...
      {onDelete && (
        ...
        >
          <Trash2 size={16} />
          Удалить навык и все его задания
        </button>
      )}
    </form>
  )
