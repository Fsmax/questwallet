...
- `src/screens/` — экраны вкладок: Мой день, Личный рост (квесты + навыки), Работа (рабочие таски + регулярный доход), Финансы (кошелёк + цели + долги), Настройки
- `src/myday/`, `src/dashboard/`, `src/quests/`, `src/skills/`, `src/work/`, `src/goals/`, `src/wallet/`, `src/debts/`, `src/settings/` — компоненты по фичам
